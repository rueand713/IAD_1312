...
public class GameManager {
	...
	public static void drawFont(SpriteBatch batch, BitmapFont font, String string, int x, int y)
	{
		font.draw(batch, string, x, y);
	}
	
	public static BitmapFont getBitmapFont(Color color, float scale)
	{
	...
	public static Vector3 getTouchVector(int n)
	{
		Vector3 vectPos = new Vector3();
	    vectPos.set(Gdx.input.getX(n), Gdx.input.getY(n), 0);
	...
	public static boolean overlaps(Vector2 touch, Rectangle rectangle, boolean addPadding)
	{
		...
		float xRangeMin = button.x - (padding);
		float yRangeMax = button.y + (rectangle.getHeight() + padding);
		float yRangeMin = button.y - (padding);
